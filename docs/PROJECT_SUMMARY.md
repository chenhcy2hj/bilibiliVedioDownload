...
| **v0.1.1 实施（当前）** | **P1 许可免责 ✅ / P2 批量≤10 ✅ / P3 历史持久化 ✅ / P4 历史重试 ✅ / P5 Chromium 捆绑 ✅ / P6 Release 已发布 ✅（v0.1.1：mac 248MB / win 276MB）** | 🔵 真机验收待做 |

**下一步（接续点）**：真机验收（mac 下载 Release v0.1.1 → 右键打开 → 点「获取 Cookie」弹内置 Chromium 登录自动捕获 → 下载冒烟；Windows 同流程）→ 结果回填 `docs/manual-test-plan.md`（§4 C09、§5.5 H、§10）与 roadmap P6 勾选。发现缺陷按 §8 约定修复后推 v0.1.2。
...
8. **前端**：单文件 `store.js`（reactive）；任务分组**必须 `computed`**（踩过"计数更新列表为空"坑）；条形码进度条（下载蓝滚动/转码琥珀/100% 绿）；CookieBar 一键无感获取 + 手动方式折叠兜底。
...
# 测试与规范（每次改动必跑）
cd backend && ../venv/bin/python -m pytest tests/   # 全量用例
...
| `roadmap.md` | v0.1.1 清单+验收 / v0.1.2+ 候选 / 明确不做 | 规划版本 |
...
1. **文档优先**：实施前先读 `design-v0.1.1.md` 对应节；实现偏差 → 先更新设计文档再改代码；
