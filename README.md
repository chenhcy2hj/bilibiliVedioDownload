...
> 本项目仅为**个人学习、技术研究**而开发，**公开可用，但不对使用者负责**。
> 下载/使用/分发本项目或其构建产物，即视为您自行承担全部后果。
...
- 使用本项目产生的任何风险（包括但不限于账号风控、封禁、法律纠纷）**由使用者自行承担**，与项目作者无关；
- 输出内容仅限个人学习交流，如需公开使用，请事先获得相应权利人的授权；
- 如本项目内容涉及您的合法权益，请联系作者删除相关内容。

## 📄 License

本项目以 **MIT License** 授权（见仓库根 `LICENSE` 文件）。按 MIT 授权条款，**本项目按"现状"提供，作者不对使用后果（含上述风控与合规风险）承担任何责任**。
